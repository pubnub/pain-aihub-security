...
app.post('/login', (req, res) => {
  ...
  request.post(loginOptions, (err, res1, body) => {
    if (err || !res1 || !res1.body || !res1.body.result) {
      console.error(`🚫 Error: ${err}`);

      if (err && err.message.includes('Server returned nothing')) {
        console.error(`⌛ Request to ${loginOptions.url} timed out: ${err}`);
      }

      return res.status(500).send({
        error: 'The request to the authentication server failed. This might be due to a network error, the server being down, or an unexpected response. Please check your VPN connection. 😞'
      });
    }
    ...
    console.log("📘 Account Options:", accountsOptions);

    request.get(accountsOptions, (err, res2, body2) => {
      if (err || !res2 || !body2) {
        console.error(`🚫 Error: ${err}`);
        return res.status(500).send({
          error: 'There was an error fetching the accounts. Please try again later. 😞'
        });
      }
      ...
      try {
        ...
        return res.status(500).send({
          error: 'There was an error parsing the accounts response. Please try again later. 😞'
        });
      }
